  ...
  template: `
    ...
    >
      <div class="mb-5 flex items-center gap-3">
        ...
        <div>
          <p class="text-xs font-semibold uppercase tracking-wide text-gray-500">Sales terminal</p>
          <h2 class="text-xl font-bold text-gray-900">Create Order</h2>
        </div>
      </div>
      ...
      <div class="grid gap-4">
        <label class="grid gap-1 text-sm font-medium text-gray-700">
          Customer / token note
          <input
            class="min-w-0 rounded-xl border border-gray-300 px-3 py-2 text-sm outline-none focus:border-primary"
        ...
        <div class="grid gap-3">
          <div
            class="hidden grid-cols-[minmax(0,1fr)_84px_128px_44px] gap-2 px-2 text-xs font-semibold uppercase tracking-wide text-gray-500 min-[900px]:grid"
          >
            <span>Item</span>
            <span>Qty</span>
          ...
          @for (item of items(); track item.id) {
            <div
              class="grid min-w-0 gap-2 rounded-2xl bg-gray-100 p-2 min-[900px]:grid-cols-[minmax(0,1fr)_84px_128px_44px]"
            >
              <input
                class="min-w-0 rounded-xl border border-gray-300 bg-white px-3 py-2 text-sm outline-none focus:border-primary"
                [ngModel]="item.name"
              ...
              <button
                type="button"
                class="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border border-gray-300 bg-white text-gray-500 transition hover:border-danger hover:text-danger"
        ...
        <div
          class="flex flex-col gap-3 rounded-3xl bg-primary-subtle p-4 min-[640px]:flex-row min-[640px]:items-center min-[640px]:justify-between"
        >
          <div class="min-w-0 min-[640px]:basis-2/3">
            <p class="text-xs font-semibold uppercase tracking-wide text-primary">Total</p>
          ...
          <p
            class="min-w-0 text-sm leading-5 text-gray-700 min-[640px]:basis-1/3 min-[640px]:text-right"
          >
            @if (hasValidItems()) {
              Ready for checkout.
            } @else {
        ...
        <div class="flex flex-wrap gap-2">
          <ds-button variant="secondary" size="md" (clicked)="addItem()">
            <i aria-hidden="true" class="fa-solid fa-plus text-primary"></i>
            Add Item
          </ds-button>
    ...
    @if (isCheckoutOpen()) {
      ...
      <section
        ...
        class="fixed left-1/2 top-1/2 z-[1001] max-h-[92vh] w-[min(640px,92vw)] -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-[2rem] bg-white p-6 text-gray-900 shadow-2xl"
      >
        <header class="flex items-start justify-between gap-4">
          ...
          <button
            type="button"
            class="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-gray-500 transition hover:bg-gray-100 hover:text-gray-900"
        ...
        <div class="mt-6 rounded-3xl bg-gray-100 p-4">
          <div class="flex items-center justify-between gap-3">
            ...
            <span
              class="inline-flex h-12 w-12 items-center justify-center rounded-2xl bg-white text-primary shadow-sm"
            >
          ...
          <div class="mt-4 border-t border-gray-300 pt-4">
            <div class="flex items-center justify-between gap-3">
              <span class="text-sm font-semibold text-gray-700">Total payable</span>
        ...
        <div class="mt-6 grid gap-3">
          <p class="text-sm font-semibold text-gray-900">Payment method</p>
          ...
          <div class="grid gap-3 sm:grid-cols-2">
            ...
            >
              <span
                class="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-white text-primary shadow-sm"
              >
                <i aria-hidden="true" class="fa-solid fa-money-bill-wave"></i>
              </span>
              <span class="min-w-0 text-left">
                <span class="block font-bold">Manual / Cash</span>
                <span class="mt-1 block text-xs text-gray-500">Default for now</span>
              </span>
            </button>
            ...
            >
              <span
                class="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-white text-info shadow-sm"
              >
                <i aria-hidden="true" class="fa-solid fa-credit-card"></i>
              </span>
              <span class="min-w-0 text-left">
                <span class="block font-bold">Card / NFC</span>
                <span class="mt-1 block text-xs text-gray-500">Coming soon</span>
              </span>
            </button>
          </div>
        </div>
        ...
        <label
          class="mt-5 flex cursor-pointer items-center justify-between gap-4 rounded-3xl border border-gray-300 bg-white p-4"
        >
          <span class="flex min-w-0 items-center gap-3">
            ...
            <span class="min-w-0">
              <span class="block text-sm font-bold text-gray-900">Print receipt</span>
        ...
        @if (selectedPaymentMode() === 'card') {
          <div
            class="mt-5 rounded-3xl border border-warning bg-warning-subtle p-4 text-sm text-gray-700"
          >
...
export class OrderCreationPanelComponent {
  ...
  paymentModeClass(mode: PaymentMode): string {
    ...
    const selectedClass =
      mode === 'manual' ? 'border-primary bg-primary-subtle' : 'border-info bg-info-subtle';

    return [
      'flex min-w-0 items-center gap-3 rounded-3xl border p-4 text-left transition hover:-translate-y-0.5 hover:shadow-sm',
      selected ? selectedClass : 'border-gray-300 bg-white',
    ].join(' ');
  }
